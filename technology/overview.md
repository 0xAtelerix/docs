...
> This results in real-time processing of application-specific logic: significantly boosting performance and reducing latency.
...
* Enables reactive smart contracts that respond in near real-time to external events and data: fostering a dynamic and responsive execution environment for applications.
...
* [Validator network](overview.md#pelagos-validator-network) operating the DAG consensus layer
* [Application logic layer](overview.md#application-logic-layer)
* [Reactive smart contracts](overview.md#reactive-smart-contracts)
* Dedicated [tokenomics API](overview.md#tokenomics-as-a-firstclass-primitive)
...
Pelagos validators provide a crosschain data access layer that cryptographically ensures the integrity of the data that applications rely on. Pelagos validators operate nodes for all supported blockchains as well as the Pelagos DAG chain. This guarantees constant access to up-to-date, data from multiple L1s and L2s.

As the head of a supported external chain updates, validators submit and confirm it within Pelagos’ DAG consensus process. Once a sufficient quorum is reached, this external block and all its data (state, events, balances, transactions) become available to all Pelagos applications as a native, trust-minimized data source available for querying or data aggregation.
...
> See more on the [validator role and responsibilities](validating-applications.md#validating-applications-with-pelagos).
...
Instead of manually polling for updates, an application can trigger automatically, executing specified logic when certain conditions are met on or offchain. This capability allows applications to implement near real-time responses to market changes, network events, or other external data sources, enabling a more seamless and efficient multichain user experience.
...
Reactive contracts are secure and can operate with near-instant finality at the application level.

> See more on [event-based triggers](reactive-logic.md).

## Tokenomics as a first-class primitive

The Pelagos API model facilitates maximum tokenomics flexibility. Furthermore, applications built on Pelagos can design tokenomics that operate on a multichain level, rather than being confined to a single blockchain.
