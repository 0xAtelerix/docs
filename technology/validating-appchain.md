...
To operate as a validator requires participants to run application execution nodes by downloading the Docker container and syncing the latest application state via torrent-based immutable databases.
...
### Validator application lifecycle management

Validators vote to confirm application readiness for start, stop, or halt events based on user subscription status and consensus. They also participate in checkpoint formation and commit aggregate application epoch results and immutable DB info-hashes on the supported L1/L2.
...
Each validator is responsible for operating application containers with resource isolation. Furthermore, the validator role includes a DevOps role as validators are expected to monitor and react to excessive resource consumption with container restarts or by disabling unstable applications to protect network health. This ensures the overall health of the network by stabilizing resource consumption and maintaining stability of application execution environments to avoid forced shutdowns.
...
* Failure to execute application logic.
...
By combining economic incentives, cryptographic guarantees, and strict enforcement, Pelagos ensures validators uphold the trust and security that make scalable applications possible.
