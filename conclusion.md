...
* Scalablilty as a first-class primitive: allowing each application to scale according to its own needs, rather than forcing all projects into a one-size-fits-all model.

Within the Pelagos protocol, data is always available, trustworthy, and native-level for any supported blockchain — removing the need for lending trust to oracles or relying on fragile bridges.
